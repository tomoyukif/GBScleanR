...
Then, get histograms using `histGBSR()`.
```{r}
...
```{r}
gdata <- setSnpFilter(gdata, maf = 0.25)
nsnp(gdata)
```
\  
...
To execute genotype error correction, we first need to build a scheme object. Our simulation data was a biparental F2 population. Therefore, we should run `initScheme()` and `addScheme()` as following.
```{r}
gdata <- initScheme(gdata, crosstype = "pairing", mating = matrix(1:2, 2))
...
The function `gbsrGDS2VCF()` close the connection to the GDS file of the input GbsrGenotypeData object, internally. Thus, you need to reopen the connection if needed.
```{r eval = FALSE}
loadGDS(genotypeData = gdata)
```
