...
- `gdsfmt`  
- `biobase`  
- [`GWASTools`](https://bioconductor.org/packages/release/bioc/html/GWASTools.html)(ref1)
- [`SeqArray`](https://bioconductor.org/packages/release/bioc/html/SeqArray.html)(ref2 and ref3)
\  
...
GBScleanR only support a VCF file as input. As an example data, we use sample genotype data for a biparental F2 population derived from inbred founders.
```{r}
...
head(getChromosome(gdata)) 
```
```{r}
# Chromosome names of all markers
head(getChromosome(gdata, name = TRUE)) 
```
```{r}
# Unique set of chromosome names
getChromosome(gdata, levels = TRUE) 
```
```{r}
# Position (bp) of all markers
head(getPosition(gdata)) 
```
```{r}
# Reference allele of all markers
head(getAlleleA(gdata)) 
```
```{r}
# Alternative allele of all markers
head(getAlleleB(gdata)) 
```
```{r}
# SNP IDs
head(getSnpID(gdata)) 
...
histGBSR(gdata, stats = "qtile_ref", q = 0.5) 
```
...
head(getCountGenoRef(gdata, target = "snp")) 
# Reference genotype count per sample
head(getCountGenoRef(gdata, target = "scan")) 
```
```{r}
# Heterozygote count per marker
head(getCountGenoHet(gdata, target = "snp")) 
# Heterozygote count per sample
head(getCountGenoHet(gdata, target = "scan")) 
```
```{r}
# Alternative genotype count per marker
head(getCountGenoAlt(gdata, target = "snp")) 
# Alternative genotype count per sample
head(getCountGenoAlt(gdata, target = "scan"))
```
```{r}
# Missing count per marker
head(getCountGenoMissing(gdata, target = "snp")) 
# Missing count per sample
head(getCountGenoMissing(gdata, target = "scan")) 
```
```{r}
# Reference allele count per marker
head(getCountAlleleRef(gdata, target = "snp")) 
# Reference allele count per sample
head(getCountAlleleRef(gdata, target = "scan")) 
```
```{r}
# Alternative allele count per marker
head(getCountAlleleAlt(gdata, target = "snp")) 
# Alternative allele count per sample
head(getCountAlleleAlt(gdata, target = "scan")) 
```
```{r}
# Missing allele count per marker
head(getCountAlleleMissing(gdata, target = "snp")) 
# Missing allele count per sample
head(getCountAlleleMissing(gdata, target = "scan")) 
```
```{r}
# Reference read count per marker
head(getCountReadRef(gdata, target = "snp")) 
# Reference read count per sample
head(getCountReadRef(gdata, target = "scan")) 
```
```{r}
# Alternative read count per marker
head(getCountReadAlt(gdata, target = "snp")) 
# Alternative read count per sample
head(getCountReadAlt(gdata, target = "scan")) 
```
```{r}
...
head(getMAF(gdata, target = "snp")) 
# Minor allele frequency per sample
head(getMAF(gdata, target = "scan")) 
```
```{r}
# Minor allele count per marker
head(getMAC(gdata, target = "snp")) 
# Minor allele count per sample
head(getMAC(gdata, target = "scan")) 
```
\  
...
Here, let's filter out calls supported by less than 5 reads and then filter out markers having more than 20% of missing rate.
```{r}
gdata <- setCallFilter(gdata, dp_count = c(5, Inf))
gdata <- setSnpFilter(gdata, missing = 0.2)
```
\  
...
```
\    
If you have already loaded the GDS file in the current R session, the command above will return an error. In that case, please close the connection first and then load again or reload using `loadGDS()` with specifying `genotypeData`.
```{r}
closeGDS(subset_gdata)
subset_gdata <- loadGDS(subset_fn)
subset_gdata <- loadGDS(genotypeData = subset_gdata)
```
...
\section{References}
ref1: Laurie, C. C., Doheny, K. F., Mirel, D. B., Pugh, E. W., Bierut, L. J., Bhangale, T., Boehm, F., Caporaso, N. E., Cornelis, M. C., Edenberg, H. J., Gabriel, S. B., Harris, E. L., Hu, F. B., Jacobs, K. B., Kraft, P., Landi, M. T., Lumley, T., Manolio, T. A., McHugh, C., Painter, I., Paschall, J., Rice, J. P., Rice, K. M., Zheng, X., and Weir, B. S., for the GENEVA Investigators (2010), Quality control and quality assurance in genotypic data for genome-wide association studies. Genetic Epidemiology, 34: 591-602. doi: 10.1002/gepi.20516

ref2: Zheng X, Gogarten S, Lawrence M, Stilp A, Conomos M, Weir B, Laurie C, Levine D (2017). “SeqArray – A storage-efficient high-performance data format for WGS variant calls.” Bioinformatics. doi: 10.1093/bioinformatics/btx145. 

ref3: Zheng X, Levine D, Shen J, Gogarten S, Laurie C, Weir B (2012). “A High-performance Computing Toolset for Relatedness and Principal Component Analysis of SNP Data.” Bioinformatics, 28(24), 3326-3328. doi: 10.1093/bioinformatics/bts606. 
