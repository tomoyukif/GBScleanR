---
title: "Basic usage of utility functions in GBScleanR"
...
hist(gdata, stats = "missing") # Histgrams of missing rate
```
```{r fig.cap="Heterozygosity per marker and per sample.", out.height="35%"}
hist(gdata, stats = "het") # Histgrams of heterozygosity
```
```{r fig.cap="Reference allele frequency per marker and per sample.", out.height="35%"}
hist(gdata, stats = "raf") # Histgrams of reference allele frequency
```
\  
...
hist(gdata, stats = "dp") # Histgrams of total read depth
```
```{r fig.cap="Reference read depth per marker and per sample.", out.height="35%"}
hist(gdata, stats = "ad_ref") # Histgrams of allelic read depth
```
...
hist(gdata, stats = "rrf") # Histgrams of reference allele frequency
```
\  
...
hist(gdata, stats = "mean_ref") # Histgrams of mean allelic read depth
```
...
hist(gdata, stats = "sd_ref") # Histgrams of standard deviation of read depth
```
...
hist(gdata, stats = "qtile_ref", q = 0.5) # Histgrams of quantile of read depth
```
...
```{r}
plot(gdata, stats = "missing", coord = c(6, 2)) # coord controls the number of rows and columns of facets.
```
```{r}
