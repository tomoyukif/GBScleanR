...
This sample data contains 813 of F2 individuals derived from the cross between Oryza sativa cv. Nipponbare and O. longistaminata, which is a cross between distant relatives. The parental lines and one F1 individual are also included in the data.  
We can omit the F1 individual which will not be used in the downstream genetic analysis.
```{r}
f1 <- grep("F1", getScanID(gdata), value = TRUE)
f1
```
```{r}
gdata <- setScanFilter(gdata, id = f1)
nScan(gdata)
```
\  
...
```{r}
p1 <- grep("NB", getScanID(gdata), value = TRUE)
p2 <- grep("OL", getScanID(gdata), value = TRUE)
...
```
```{r}
hist(gdata, stats = "het")
```
```{r}
hist(gdata, stats = "raf")
```
\  
...
```{r}
pairs(gdata, stats1 = "mean_ref", stats2 = "sd_ref", target = "snp", smooth = TRUE)
```
```{r}
pairs(gdata, stats1 = "mean_ref", stats2 = "sd_ref", target = "snp", smooth = TRUE,
...
```
```{r}
pairs(gdata, stats1 = "mean_alt", stats2 = "sd_alt", target = "snp", smooth = TRUE)
```
\  
...
cowplot::plot_grid(p1, p2, ncol = 1, rel_heights = c(3, 1), align = "v", axis = "lr")
```
\  
...
```{r}
plot(gdata, stats = "raf", coord = c(6, 2))
```
\  
...
```{r}
# Marker density
plot(gdata, stats = "marker", coord = c(6, 2))
```
```{r}
plot(gdata, stats = "geno", coord = c(6, 2))
```
...
```{r}
gdata <- loadGDS("./data/gbs_nbolf2_subset.gds")
```
