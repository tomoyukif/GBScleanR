...
.convertGDS <- function(gds_fn) {
  ...
  new_ano <-
    gdsfmt::addfolder.gdsn(node = new_gds, name = "annotation")
  gdsfmt::copyto.gdsn(
    node = new_ano,
  ...
  if (!class(ls_format) %in% "try-error") {
    ...
    if (length(ls_format) != 0) {
      new_format <-
        gdsfmt::addfolder.gdsn(node = new_ano,
                               name = "format",
                               replace = TRUE)
      for (i in ls_format) {
        ...
        new_sub <-
          gdsfmt::addfolder.gdsn(node = new_format,
                                 name = label,
                                 replace = TRUE)
        newdata <- gdsfmt::add.gdsn(
          node = new_sub,
          name = "data",
          replace = TRUE,
  ...
  chr_node <-
    gdsfmt::index.gdsn(node = new_gds, path = "snp.chromosome")
  new_chr_node <- gdsfmt::add.gdsn(
    node = new_gds,
...
.buildSnpAnnot <- function(gds) {
  ...
  if ("info/QD" %in% ls_gdsn) {
    pdata <- cbind(pdata,
                   QD = gdsfmt::read.gdsn(node = gdsfmt::index.gdsn(
                     node = gds@handler,
                     path =
                       "info/QD"
                   )))
  ...
  if ("info/FS" %in% ls_gdsn) {
    pdata <- cbind(pdata,
                   FS = gdsfmt::read.gdsn(node = gdsfmt::index.gdsn(
                     node = gds@handler,
                     path =
                       "info/FS"
                   )))
  ...
  if ("info/SOR" %in% ls_gdsn) {
    pdata <- cbind(pdata,
                   SOR = gdsfmt::read.gdsn(node = gdsfmt::index.gdsn(
                     node = gds@handler,
                     path =
                       "info/SOR"
                   )))
  ...
  if ("info/MQ" %in% ls_gdsn) {
    pdata <- cbind(pdata,
                   MQ = gdsfmt::read.gdsn(node = gdsfmt::index.gdsn(
                     node = gds@handler,
                     path =
                       "info/MQ"
                   )))
  ...
  if ("info/MQRankSum" %in% ls_gdsn) {
    pdata <- cbind(pdata,
                   MQRankSum = gdsfmt::read.gdsn(
                     node = gdsfmt::index.gdsn(node = gds@handler,
                                               path =
                                                 "info/MQRankSum")
                   ))
  }
  if ("info/ReadPosRankSum" %in% ls_gdsn) {
    pdata <- cbind(pdata,
                   ReadPosRankSum = gdsfmt::read.gdsn(
                     node = gdsfmt::index.gdsn(node = gds@handler,
                                               path =
                                                 "info/ReadPosRankSum")
                   ))
  ...
  return(GWASTools::SnpAnnotationDataFrame(pdata))
}
...
.buildScanAnnot <- function(gds) {
  ...
  return(GWASTools::ScanAnnotationDataFrame(data.frame(scanID,
                                                       validScan,
                                                       stringsAsFactors = FALSE)))
}
...
#' @importMethodsFrom GWASTools getSnpVariable getSnpAnnotation
...
loadGDS <- function(gds_fn, non_autosomes = NULL, genotypeData) {
  if (missing(genotypeData)) {
    ...
    gds <-
      GWASTools::GdsGenotypeReader(filename = gds_fn,
    ...
    if (is.null(non_autosomes)) {
      ...
      n_chr <-
        length(unique(
          GWASTools::getSnpVariable(object = out, varname = "chromosome")
        ))
    ...
    gds <-
      GWASTools::GdsGenotypeReader(filename = gds_fn, genotypeDim = "scan,snp")
  ...
  out@data@handler <- gdsfmt::openfn.gds(
