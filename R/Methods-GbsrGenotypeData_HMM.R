...
.pat2hap <- function(best_hap, param_list) {
  ...
  valid_scan_i <- c(param_list$parents_index, which(param_list$samples_index))
  out_hap <- array(out_hap, c(param_list$n_ploidy,
                              param_list$n_snp,
                              length(valid_scan_i)))
  out_hap[, , valid_scan_i] <- out_hap
  return(out_hap)
}
...
.summarizeEst <-
  function(best_hap, best_geno, pat_prob, param_list) {
    ...
    sample_geno <- as.vector(best_geno[, param_list$samples_index] + 1)
...
.runCycle <- function(param_list, outprob, outgeno) {
  ...
  if (outgeno) {
    ...
    best_pat <- .halfJoint(best_pat_f, best_pat_r,
                           param_list$n_snp, param_list$n_samples)
