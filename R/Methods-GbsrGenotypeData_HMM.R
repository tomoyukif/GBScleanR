          ...
          function(object,
            ...
            for (chr_i in chr_all) {
              ...
              if (!chr_i %in% chr) {
                ...
                msg <- paste0("Chr ", chr_i, " was skipped.")
                message(msg)
                next
              }
...
.loadReadCounts <- function(object, index) {
  ...
  ad <-
    gdsfmt::readex.gdsn(ad_node, list(valid_scan, valid_markers_2))
  ref <- ad[, c(TRUE, FALSE)]
  alt <- ad[, c(FALSE, TRUE)]
...
.getJnum <- function(scheme, het_parent) {
  ...
  if (n_pairing == 0) {
    ...
    jnum <-
      .initJnum(n_founder, n_pairing + het_parent, next_crosstype)
  }
...
.initJnum <- function(n_founder, n_pairing, next_crosstype) {
  ...
  if (!missing(n_pairing)) {
    ...
    if (next_crosstype == "selfing") {
      ...
      jnum$j1232 <-
        jnum$r <- jnum$k1232 <- switch(n_pairing, "1" = 0, n_pairing - 2)
      if (n_founder > 2) {
        jnum$a123 <- 1
      } else {
        jnum$b12 <- 0.5
...
.getValidPat <- function(scheme, het_parent, n_origin) {
  ...
  if (het_parent) {
    pat <-
      apply(matrix(seq_len(n_origin), nrow = 2), 2, paste, collapse = "/")
    chr <- pat[match(scheme@mating[[1]], seq_len(0.5 * n_origin))]
    if (scheme@crosstype[1] == "pairing") {
      pat <-
        apply(matrix(chr, dim(scheme@mating[[1]])), 2, paste, collapse = "|")
      
    } else if (scheme@crosstype[1] == "random") {
      mating <- t(expand.grid(chr, chr))
      mating <-
        mating[, apply(mating, 2, function(x)
          length(unique(x)) != 1)]
  ...
  for (i in 2:length(scheme@crosstype)) {
    if (scheme@crosstype[i] == "pairing") {
      chr <- sub("\\|", "/", pat)
      mating <-
        matrix(chr[match(scheme@mating[[i]], scheme@progenies[[i - 1]])],
               dim(scheme@mating[[i]]))
      pat <- apply(mating, 2, paste, collapse = "|")
      ...
      chr <- strsplit(pat, "\\|")[[1]]
      pat <-
        apply(t(expand.grid(chr, chr)), 2, paste, collapse = "|")
      homo <- TRUE
  ...
  for (i in seq_len(chr)) {
    ...
    pat <-
      rbind(pat, expand.grid(chr1, chr2, stringsAsFactors = FALSE))
  }
  ...
  if (homo) {
    pat <-
      rbind(pat, t(matrix(rep(
        unique(as.vector(pat)), each = 2
      ), 2)))
  }
  pat_dim <- dim(pat)
...
.transitionProb <-
  function(pat,
    ...
    ibd <-
      apply(pat$hap_progeny, 1, function(x)
        abs(length(unique(x)) - 2))
    ibd <-
      vapply(ibd, function(x)
        paste0(ibd, x), character(length(ibd)))
    ...
    q_mat <-
      matrix(q_mat, nrow(pat$hap_progeny), nrow(pat$hap_progeny))
    diag(q_mat) <- NA
    diag(q_mat) <- -rowSums(q_mat, na.rm = TRUE)
    prob <- vapply(rf, function(x) {
...
.addParents <- function(best_hap, p_geno, param_list) {
  ...
  index <-
    c(which(param_list$samples_index), param_list$parents_index)
...
.saveHap <- function(object, best_hap, n_snp, valid_index) {
  if (is.array(best_hap)) {
    ...
    output[, valid_index, getValidScan(object, parents = TRUE)] <-
      best_hap
  ...
  hap_gdsn <-
    gdsfmt::index.gdsn(object@data@handler, "estimated.haplotype")
...
.pat2hap <- function(best_hap, param_list) {
  ...
  if (param_list$het_parent) {
    parent_hap <- rep(seq_len(n_parents_chr),
                      each = param_list$n_snp)
  } else {
    parent_hap <- rep(
      seq(1, by = 2, length.out = param_list$n_parents),
      each = param_list$n_ploidy * param_list$n_snp
    )
  }
  parent_hap <-
    matrix(parent_hap, nrow = n_parents_chr, byrow = TRUE)
  sample_pat <- best_hap$best_seq
  sample_hap <- apply(sample_pat, 1, function(x) {
    return(rbind(
      param_list$pat$hap_progeny[x, 1],
  ...
  out_hap <- apply(out_hap, 2, function(x)
    return(x))
  out_hap <- array(out_hap,
                   c(
                     param_list$n_ploidy,
...
.savePGeno <- function(object, p_geno, n_snp, valid_index) {
  ...
  out_gdsn <-
    gdsfmt::index.gdsn(object@data@handler, "parents.genotype")
  gdsn_dim <- gdsfmt::objdesp.gdsn(out_gdsn)$dim
  if (gdsn_dim[1] == 0) {
    gdsfmt::add.gdsn(
      node = object@data@handler,
...
.makePattern <- function(n_parents,
  ...
  valid_pat <-
    apply(geno_parents, 1, function(x) {
      length(unique(x)) > 1
    })
  geno_parents <- as.matrix(geno_parents[valid_pat,])
  if (!het_parent) {
    valid <-
      apply(geno_parents[, c(TRUE, FALSE)] == geno_parents[, c(FALSE, TRUE)], 1, all)
  ...
  possiblehap <-
    rbind(derived_geno, derived_geno, derived_geno) == geno_pat
  ...
  possiblegeno <-
    rbind(possiblegeno, possiblegeno, possiblegeno) == geno_pat
...
.sumUpBias <- function(bias1, bias2) {
  ref_prop <-
    (bias1$ref + bias2$ref) / (bias1$n_ref + bias2$n_ref * 2)
  alt_prop <-
    (bias1$alt + bias2$alt) / (bias1$n_alt + bias2$n_alt * 2)
  bias <- ref_prop / (ref_prop + alt_prop)
  return(bias)
}
...
.summarizeEst <-
  function(best_hap, best_geno, pat_prob, param_list) {
    n_snp <- param_list$n_snp
    n_sample <- param_list$n_samples
    sample_geno <- best_geno[, param_list$samples_index] + 1
    sample_geno <-
      sample_geno + seq(from = 0,
                        by = 3,
                        length.out = n_snp * n_sample)
    geno_prob <-
      matrix(pat_prob[sample_geno], n_snp, n_sample) < log10(param_list$call_threshold)
...
.runCycle <- function(param_list, outprob, outgeno) {
  ...
  msg <- paste0("\r", cycle, "Backward path...")
  message(msg)
  if (param_list$flip) {
    best_pat_f <- .getBestSeq(param_list, outprob)
    best_hap_f <- .pat2hap(best_pat_f, param_list)
  ...
  if (!outprob) {
    msg <- paste0("\r", cycle, "Estimating error pattern...")
    message(msg)
    error_f <- .calcErrors(best_geno_f, param_list)
    error_r <-
      .calcErrors(best_geno_r[param_list$n_snp:1,], param_list)
  ...
  if (outgeno) {
    msg <- paste0("\r", "Summarizing output...")
    message(msg)
    ...
    out_list <-
      .summarizeEst(best_hap, best_geno, best_pat$prob, param_list)
...
.checkPread <- function(param_list) {
  ...
  p_read_s <-
    sum(param_list$reads$p_ref[, 1] == 0 &
          param_list$reads$p_alt[, 1] == 0)
  p_read_e <-
    sum(param_list$reads$p_ref[, param_list$n_snp] == 0 &
          param_list$reads$p_alt[, param_list$n_snp] == 0)
  if (p_read_s < p_read_e) {
    param_list$flip = TRUE
  } else if (p_read_s == p_read_e) {
    p_read_s <-
      sum(param_list$reads$p_ref[, 1] + param_list$reads$p_alt[, 1])
    p_read_e <-
      sum(param_list$reads$p_ref[, param_list$n_snp] + param_list$reads$p_alt[, param_list$n_snp])
    if (p_read_s < p_read_e) {
      param_list$flip = TRUE
    }
  }
